...
MODEL SELF-CHECK — do this before anything else. Your system prompt names the model you run on. Your first output line must be exactly `reviewed-by-model: <model id>`. If that id — ignoring a trailing context-window suffix such as `[1m]`, which is not part of the id — is not `claude-fable-5-1` — the review floor, which changes ONLY by the coordinated four-place bump in docs/WORKFLOW.md § "Version check" (this line, `$reviewFloor` in `.claude/hooks/sprint-close-guard.ps1`, the fixtures in `.claude/hooks/test-sprint-close-guard.ps1` and the reference list, in one commit; a floor named only in a prompt is not the floor, because the close gate would refuse it) — write `verdict: REFUSED — wrong model for review` as the second line and STOP. Planning and review run on the newest Fable by owner ruling (2026-09-07, made precise 2026-09-24); a cheaper reviewer silently lowers the bar the whole workflow rests on.
