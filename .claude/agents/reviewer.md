...
MODEL SELF-CHECK — do this before anything else. Your system prompt names the model you run on. Your first output line must be exactly `reviewed-by-model: <model id>`. If that id is not `claude-fable-5-1` (or a successor the Orchestrator's prompt explicitly names as the review floor), write `verdict: REFUSED — wrong model for review` as the second line and STOP. Planning and review run on the newest Fable by owner ruling (2026-09-07, made precise 2026-09-24); a cheaper reviewer silently lowers the bar the whole workflow rests on.
