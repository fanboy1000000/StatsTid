...
| [docs/operations/hr-follow-up-process-register.md](docs/operations/hr-follow-up-process-register.md) | The HRP-series register of every process the system hands HR/admins to follow up on (S139 analysis): one row per hand-off — trigger, accountable role, deadline + source, surfacing today, aging, resolution + audit, decision-readiness — plus the Increment-4 design inputs and the owner's shape ruling. **Add a row when a new hand-off is designed**; re-read a row when its process changes ("revisit, not shield") |
| [docs/operations/model-routing-register.md](docs/operations/model-routing-register.md) | One row per sprint on model use vs. review/CI outcomes — the thin monitor for the model-routing rule (planning and review on the most capable model, implementation on cheaper ones; `docs/WORKFLOW.md` § Model Routing). **Add the sprint's row at close**; read for direction over three sprints |
