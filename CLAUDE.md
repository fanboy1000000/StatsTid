...
| [docs/operations/model-routing-register.md](docs/operations/model-routing-register.md) | One row per sprint on model use vs. review/CI outcomes — the thin monitor for the model-routing rule (planning and review on the newest Fable, implementation on Opus and cheaper; `docs/WORKFLOW.md` § Model Routing). **Add the sprint's row at close**; read for direction over three sprints |
