...
- **Phase 4d-1: Wage-type-mapping versioned history** — split 2026-05-09 into design + implementation due to deferred-refinement thrash (per `feedback_thrash_defer_real_world.md`):
  ...
  - **Sprint 29 (Phase 4d-1 implementation) — IN PROGRESS** (opened 2026-05-10 atop `8950893`). Implements ADR-020-settled design. Folds in the S20 carry-forward "WTM snapshot for full replay determinism" (`MapSegmentToExportLinesAsync` currently reads live DB during export-line stage). 11 tasks across 3 phases (Phase 1 plumbing sequential; Phase 2 endpoint+payroll worktree-parallel; Phase 3 docs+sprint-plumbing sequential). Refinement at `.claude/refinements/REFINEMENT-s29-phase-4d1.md` ran 3 dual-lens cycles with cycle-3 lens convergence (first cycle-3-converging-finite case per `feedback_thrash_defer_real_world.md`). User adjudications during refinement: (i) `GetByKeyAtAsync` replicates `(position = @position OR position = '')` position-fallback at dated read; (ii) `EmploymentProfile? profile` reaches `PeriodPlanner.Plan()` via parameter (NOT `BoundarySources` extension); (iii) **same-day-only-edit validator** at endpoint (rejects `effective_from != today` with 422 — symmetric forbid; future-dated PUT support deferred to Phase 4d-2 along with the broader effective_from temporality contract for entitlement_configs + employees). Step 0b plan review absorbed cycle 1 (Codex 1B/2W/1N + Reviewer 0B/4W/10N — all mechanical fixes, no architectural BLOCKERs). Sprint test target 808 (526 unit + 35 plain regression + 159 Docker-gated + 88 frontend; +13 net new from S28's 795 baseline).
