...
- **D2.2 ETag/If-Match propagation** (ADR-019 placeholder, S21 D2.2 carry-forward, projected ~Sprint 25): row-version + ETag pattern propagated across `agreement_configs` (DRAFT edits), `position_overrides`, `wage_type_mappings`, `entitlement_configs`. Each surface gains an `If-Match`/`If-None-Match` contract on its mutating endpoint(s); `OptimisticConcurrencyException` maps to 412 with `(expectedVersion, actualVersion, currentState)` body. End-exclusive `effective_to` semantics for any history-shaped surface. Per S24 refinement Q4 (A1 decision): the 3 D2.2-targeted repos (`AgreementConfig`, `PositionOverride`, `WageTypeMapping`) accept a second signature break in S25 (`expectedVersion` parameter + structured return) rather than pre-locking joint shape in S24.
