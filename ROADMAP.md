...
- **TASK-2206 redo** (S22 Option C carry-forward): 7-repo `(conn, tx)` overload refactor + ~21 endpoint-site conversion to use S22's single-tx atomic pattern. Touches `ApprovalPeriodRepository`, `AgreementConfigRepository`, `PositionOverrideRepository`, `WageTypeMappingRepository`, `OvertimePreApprovalRepository`, `OvertimeBalanceRepository`, `TimerSessionRepository`, plus the endpoint sites that consume them. (`EntitlementBalanceRepository` was originally the 8th repo but is excluded — its only event-emitting consumer is `SkemaEndpoints.cs:439` which is deferred to Phase 4c.5; the overload travels with Skema there to avoid shipping dead code.) Reference shape: the 4 batch commits on `worktree-agent-a9b76f8d1f88717ff` (`c615dba` / `1466af9` / `c0093e6` / `2aa3044`) preserved for site-shape but **NOT to be merged** — they violate atomicity by introducing a separate tx wrapping only the outbox enqueue.
