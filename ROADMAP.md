...
- **Phase 4d-3 Part 2 IMPLEMENTATION: Employee-profile versioning emission + rule-engine cutover + planner-snapshot** — Sprint 33 candidate. Implements ADR-023 D1-D8. ~11 tasks: new `EmploymentProfileResolver` + DI wiring; `EmployeeProfileRepository.SupersedeAndCreateAsync` (ADR-020 D2 3-case routing) + `SoftDeleteAsync` (version unchanged); new `EmployeeProfileEndpoints.DELETE` (HROrAbove + OrgScopeValidator + ADR-019 admin-strict If-Match); PUT extends to use SupersedeAndCreateAsync + cycle-3 same-day-only-edit validator + emits `EmployeeProfileSuperseded` on cross-day; PCS.cs:326-339 segmentProfile cutover; ComplianceEndpoints + BalanceEndpoints cutover to resolver; DELETE dead TimeEndpoints `/calculate*` endpoints; new `UserAgreementCodeChanged` event registered + emitted from AdminEndpoints.PUT (no S33 consumer — Phase 4e replay-data trail); marquee `ReplayAsync_StableUnderEmployeeProfileMutation_ResultByteIdentical` on 3 dated fields; frontend "as-of-date" toggle (read-only when non-today). `agreement_code` mutation NOT covered by marquee (documented determinism gap per ADR-023 D2; Phase 4e launch-blocking fix). Refinement reopens against ADR-023 as binding contract.
