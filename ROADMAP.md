...
- **S35 domain-correctness discovery (LAUNCH-BLOCKING — added 2026-05-18)**: working a small Sprint 35 D-test fix surfaced systemic gaps. (a) AC `DefaultCompensationModel` seeded as `UDBETALING` in `init.sql:1099/1105` + `CentralAgreementConfigs.cs` AC entries inherit `"UDBETALING"` default — both **WRONG** per the actual AC overenskomst, which makes AFSPADSERING the default with payment as fallback. Same field never documented in `docs/references/danish-agreements.md`. (b) System has NO mechanism for within-OK role distinction — specialkonsulent/chefkonsulent in state sector lose contractual merarbejde compensation right per the overenskomst, but the system treats all AC employees identically and would incorrectly grant compensation. `PositionOverrideConfigs` schema covers only 4 quantitative fields; can't express "no entitlement". `User.EmploymentCategory` field exists but no rule reads it (vestigial). (c) General uncertainty about how comprehensively rules are modeled across AC/HK/PROSA — AC bug found by accident; HK/PROSA likely have similar latent issues. Plan: **Phase A inventory sprint** (agreement matrix + source register tied to cirkulære PDFs + role enumeration) → **Phase B domain-expert validation** (HR/payroll specialist sign-off) → **ADR-024 role-within-agreement modeling** (design sprint) → **3-sprint implementation** (schema + cutover + exhaustive D-tests). Governance: every OK version going forward starts as `DRAFT` with mandatory source citation; per-rule traceability comments + seed-parity tests + "unknown unknown" tests (fail if any field lacks source citation). Reference doc `danish-agreements.md` inverts: source of truth becomes agreement texts, code/DB are implementations.
