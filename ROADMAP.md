...
- **Phase 4d-3 Part 2: Employee-profile versioning emission + rule-engine cutover + planner-snapshot** — Sprint 32 candidate. Lands the S31 deferred work atomically per the P4 window-safety invariant: ADR-023 with per-field bucketing matrix (planner-snapshotted: weekly_norm_hours / part_time_fraction / position; consumption-time-lookup: TBD; last-write-wins: none in S31 schema since is_part_time dropped); PCS planner-enrollment via `IPlannerEnrollment.RegisterSnapshotContract` (ADR-020 D1 inheritance — no new pattern); ComplianceEndpoints + BalanceEndpoints + TimeEndpoints cutover to read from `employee_profiles` via dated lookup atomic with planner-snapshot; `CalculateRequest.WeeklyNormHours` + `WeeklyCalculateRequest.WeeklyNormHours` HARD-CUT from request body (Q4-reverse re-adjudicated post-S31); SUPERSEDED + SOFT_DELETED event emission; admin DELETE endpoint with soft-close; marquee `ReplayAsync_StableUnderEmployeeProfileMutation_ResultByteIdentical`; "as-of-date" UI toggle; Q3 LEAVE re-adjudication for `agreement_code` / `ok_version` / `employment_category` migration from `users`. See ADR-022 §Implications "S32 commitment list" for full 8-item enumeration. Plan-mode and Step 0b will determine if S32 fits one sprint or splits per `feedback_thrash_defer_real_world.md` (per-field bucketing matrix could be a thrash-prone fork — refinement Step 4 should probe).
