...
/// <summary>
/// S137 / TASK-13703 (ADR-040 D10) — PURE pins for
/// <see cref="ComplianceEndpoints.FirstEmployedDayInMonth"/>, the "first employed day of the
/// month" derivation the compliance check uses to decide (a) whether there is anything to check
/// at all and (b) which date to resolve the employment profile at.
///
/// <para>
...
/// 500 for the hire month and a meaningless check for a month before the hire. This helper answers
/// "which is the first day of this month the person was actually employed?" from the employment
/// window(s) — <c>null</c> meaning "no day at all — nothing to check".
/// </para>
///
/// <para>
/// Semantics under test: each window is clipped to the month with ADR-040 D1/D2 (end INCLUSIVE, a
/// <c>null</c> side unbounded), the result is the earliest clipped start across ALL windows (the
/// union), and windows that contribute no day are ignored. NOT Docker-gated — no I/O — so it runs
/// locally alongside the Unit suite (it lives here rather than in Tests.Unit only because the
/// task's file scope named this folder).
/// </para>
/// </summary>
public sealed class ComplianceWindowUnionTests
{
    ...
    {
        var first = ComplianceEndpoints.FirstEmployedDayInMonth(
            Array.Empty<EmploymentWindow>(), MonthStart, MonthEnd);
    ...
    {
        var first = ComplianceEndpoints.FirstEmployedDayInMonth(
            new[] { new EmploymentWindow(null, null) }, MonthStart, MonthEnd);
    ...
    {
        var first = ComplianceEndpoints.FirstEmployedDayInMonth(
            new[] { new EmploymentWindow(new DateOnly(2025, 1, 1), null) }, MonthStart, MonthEnd);
    ...
    public void MidMonthHire_FirstDayIsTheHireDate()
    {
        ...
        var first = ComplianceEndpoints.FirstEmployedDayInMonth(
            new[] { new EmploymentWindow(hire, null) }, MonthStart, MonthEnd);
    ...
    {
        var first = ComplianceEndpoints.FirstEmployedDayInMonth(
            new[] { new EmploymentWindow(MonthEnd, null) }, MonthStart, MonthEnd);
    ...
    public void LeaverMidMonth_FirstDayIsMonthStart()
    {
        ...
        var first = ComplianceEndpoints.FirstEmployedDayInMonth(
            new[] { new EmploymentWindow(null, new DateOnly(2026, 3, 15)) }, MonthStart, MonthEnd);
    ...
    {
        var first = ComplianceEndpoints.FirstEmployedDayInMonth(
            new[] { new EmploymentWindow(null, MonthStart) }, MonthStart, MonthEnd);
    ...
    {
        var first = ComplianceEndpoints.FirstEmployedDayInMonth(
            new[] { new EmploymentWindow(new DateOnly(2026, 5, 10), null) }, MonthStart, MonthEnd);
    ...
    public void MultipleWindows_UnionTakesEarliestContributingStart_RegardlessOfOrder()
    {
        ...
        var first = ComplianceEndpoints.FirstEmployedDayInMonth(windows, MonthStart, MonthEnd);
    ...
    public void SingleDaySpellInsideMonth_ThatDay()
    {
        ...
        var first = ComplianceEndpoints.FirstEmployedDayInMonth(
            new[] { new EmploymentWindow(day, day) }, MonthStart, MonthEnd);
