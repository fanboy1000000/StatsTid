...
public sealed class ReportingLineRepositoryTests : IAsyncLifetime
{
    ...
    private static async Task CleanupTestDataAsync(NpgsqlConnection conn)
    {
        ...
        // S94 (TASK-9406): the reporting_line_tree_settings cleanup was removed — the table is gone.
    ...
    // S94 (TASK-9406): the S50 TreeSettings + enforcement-toggle Docker-gated tests (22-25:
    // GetDefault / GetEnforcementMode / Upsert-create / Upsert-update) were DELETED — REQUIRED-mode
    // enforcement is retired (ADR-035 OQ6); `TreeSettingsRepository` no longer exists.

    // S94 (TASK-9406): tests 26-28 (TreeSettings upsert/version + population-gate) and 29
    // (ExplicitFallbackConfirmation persisted on approve) were DELETED here — REQUIRED-mode
    // enforcement is retired (ADR-035 OQ6). `TreeSettingsRepository`/`reporting_line_tree_settings`,
    // the `approval_periods.explicit_fallback_confirmation` column, and the `UpdateStatusAsync`
    // `explicitFallbackConfirmation` param no longer exist. The KEPT `approval_method` /
    // `designated_approver_id` persistence is covered by DesignatedApproverAuthorityTests.
