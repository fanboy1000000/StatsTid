// S99 / TASK-9905 — vitest for the redesigned Organisation page. Mocks the four
// consumed hooks (useOrganizationTree, useOrganizations, useOrganizationStructure,
// useEnheder) rather than fetch, then asserts: the 3-tier tree renders; the level
// control defaults to Organisation; search flattens + the empty message; each
// dialog; the status branches (delete 422 + count / create 409 dup / move 400 vs
// 422); the NO-DEAD-BUTTON matrix (Tilføj absent on Enhed; Flyt absent on MAO +
// Enhed; Omdøb inline on Enhed, dialog on MAO/Organisation); the Enhed-delete
// dialog shows NO "underenheder slettes" line; and the rename PUT body is
// NAME-ONLY (doesn't clobber agreement/ok).
...
// ── tree fixture: 1 MAO → 2 Organisations; one Organisation has 1 Enhed ──
...
vi.mock('../../../hooks/useEnheder', () => ({
  useEnheder: () => ({ fetchEnheder, createEnhed, renameEnhed, deleteEnhed }),
}))
...
beforeEach(() => {
  ...
  fetchEnheder.mockReset().mockResolvedValue({
    ok: true,
    data: [{ enhedId: 'ENH01', organisationId: 'STY01', name: 'Team Drift', version: 3, etag: '"3"' }],
  })
...
describe('OrganisationPage — no-dead-button matrix', () => {
  it('Tilføj is ABSENT on an Enhed, present on MAO + Organisation', () => {
    renderPage()
    fireEvent.click(screen.getByRole('button', { name: 'Enhed' }))
    expect(screen.queryByTestId('action-tilfoej-ENH01')).toBeNull()
    expect(screen.getByTestId('action-tilfoej-MIN01')).toBeDefined()
    expect(screen.getByTestId('action-tilfoej-STY01')).toBeDefined()
  })

  it('Flyt is ABSENT on MAO + Enhed, present on Organisation only', () => {
    renderPage()
    fireEvent.click(screen.getByRole('button', { name: 'Enhed' }))
    expect(screen.queryByTestId('action-flyt-MIN01')).toBeNull()
    expect(screen.queryByTestId('action-flyt-ENH01')).toBeNull()
  ...
  it('Omdøb is INLINE on an Enhed (no dialog) and a DIALOG on MAO/Organisation', () => {
    renderPage()
    fireEvent.click(screen.getByRole('button', { name: 'Enhed' }))
    // Enhed Omdøb → inline input, no dialog scrim.
...
describe('OrganisationPage — create dialog', () => {
  ...
  it('Organisation Tilføj creates an Enhed via createEnhed', async () => {
    renderPage()
    ...
    fireEvent.submit(input.closest('form')!)
    await waitFor(() => {
      expect(createEnhed).toHaveBeenCalledWith('STY01', 'Team Y')
    })
  })
...
describe('OrganisationPage — delete dialog (3 branches)', () => {
  ...
  it('the Enhed-delete dialog shows the untag copy and NO "underenheder slettes" line', async () => {
    renderPage()
    fireEvent.click(screen.getByRole('button', { name: 'Enhed' }))
    fireEvent.click(screen.getByTestId('action-slet-ENH01'))
    expect(screen.getByText('Slet enhed?')).toBeDefined()
    // The flat untag copy is present.
    expect(screen.getByText(/kun enhedsmærket fjernes/)).toBeDefined()
    // The S91 dead-copy guard: NO hierarchical re-parent / sub-unit promise.
    expect(screen.queryByText(/Underenheder, der slettes/)).toBeNull()
    expect(screen.queryByText(/flyttes til/)).toBeNull()
