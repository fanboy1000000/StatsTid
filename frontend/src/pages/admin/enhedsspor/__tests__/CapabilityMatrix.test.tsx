...
//   • Organisation create + Org/MAO rename ........... LocalAdmin (S98/S99)
...
// exception + vikar) so the "people-mutation surface stays absent (S109)" assertion
...
// orgCreateEnabled — the shared "+ Organisation" create on a MAO is enabled (LocalAdmin).
// orgRename      — the "Omdøb" (Org/MAO rename) affordance present (LocalAdmin).
...
const MATRIX: RoleExpect[] = [
  { role: 'Employee', actionRow: false, orgCreateEnabled: false, orgRename: false, orgMove: false, orgDelete: false, maoCreate: false },
  { role: 'LocalHR', actionRow: true, orgCreateEnabled: false, orgRename: false, orgMove: false, orgDelete: false, maoCreate: false },
  { role: 'LocalAdmin', actionRow: true, orgCreateEnabled: true, orgRename: true, orgMove: false, orgDelete: false, maoCreate: false },
  { role: 'GlobalAdmin', actionRow: true, orgCreateEnabled: true, orgRename: true, orgMove: true, orgDelete: true, maoCreate: true },
]
  ...
  for (const m of MATRIX) {
    it(`${m.role}: the exact structure-affordance set across MAO / Organisation / unit`, () => {
      ...
      if (!m.actionRow) {
        ...
        // Org mutations gate independently; a MAO is a root → never a "Flyt".
      ...
      // ── the PEOPLE-mutation surface stays ABSENT for EVERY role (S109) ───────────
      // The unit roster IS rendered read-only (leader + reports + external + vikar),
      // so this guard is load-bearing, not vacuous.
      ...
      // No person/leader row is clickable-to-edit (no person drawer), and there are
