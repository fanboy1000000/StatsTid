...
// Deltidsfraktion, Stilling, and the NEW `enhedLabel` (free-text enhed display
// label on the employee-profiles PUT). HR-gated: rendered only for an
// HROrAbove actor (the drawer hides this section for a non-HR LocalAdmin per
// OQ-5b / R3 create-path-HR-fields). Maps to the employee-profiles-row PUT.
...
export function ProfileSection({
  ...
  return (
    <section className={styles.section} aria-labelledby="ep-profile-heading">
      ...
      <div className={styles.formField}>
        <label className={styles.formLabel} htmlFor="ep-enhed">
          Enhed
        </label>
        <input
          className={styles.input}
          ...
          type="text"
          maxLength={100}
          value={fields.enhedLabel}
          onChange={setText('enhedLabel')}
          disabled={fieldsDisabled}
          title={noProfileTitle}
          placeholder="f.eks. Netværk"
          data-testid="ep-enhed"
        />
        <div className={styles.helperText}>
          Visningsetiket for medarbejderens enhed (påvirker ikke regler eller løn).
        </div>
      </div>
    </section>
  )
