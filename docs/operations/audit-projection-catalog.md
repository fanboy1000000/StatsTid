...
| `TerminationSettled` | TENANT_TARGETED | `employee → users.primary_org_id` | `employee_id` | `{employeeId, entitlementType, entitlementYear, sequence, payoutDays, modregningDays, unearnedAdvanceDays, carryoverIn, okVersion, paragraph}` (§26+§7; **emitted-no-consumer** — the Payroll consumer/lines are slice 3b) | interface (cross-process — mapper in Infrastructure, not Backend.Api) | S70 |

**Total**: 61 rows (54 through S66 + 4 S68 vacation-settlement mappers per ADR-033 slice 1a + 3 S70 termination-foundation mappers per ADR-033 slice 3a — `TerminationSettled` graduated from define-only to emitted-no-consumer in S70; the 4 remaining DEFINE-ONLY settlement events — `SettlementReversed`/`FeriehindringTransferred`/`FeriehindringPaidOut`/`SaerligeFeriedagePaidOut` — are EventSerializer-registered for replay but have NO mapper/catalog row until their automation slice).
