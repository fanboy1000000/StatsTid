...
**Total**: 67 rows (54 through S66 + 4 S68 vacation-settlement mappers per ADR-033 slice 1a + 3 S70 termination-foundation mappers per ADR-033 slice 3a + 3 S71 slice-3b mappers — `SettlementReversed` graduated from define-only to emitted in S71 — + 2 S74 manager-vikar mappers per ADR-027 Phase 5 + 1 S79 slice-4 mapper (`FeriehindringTransferred` — graduated from define-only to emitted by the §22 FERIEHINDRING resolve disposition); the 2 remaining DEFINE-ONLY settlement events — `FeriehindringPaidOut` (§25)/`SaerligeFeriedagePaidOut` (§15/§17) — are EventSerializer-registered for replay but have NO mapper/catalog row until their automation slice; the PARKED §7 `TerminationModregningApplied` is NOT defined at all — its payload shape awaits the SLS dialogue, SPRINT-71 gate (i)).
