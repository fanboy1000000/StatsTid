...
**Total**: 64 rows (54 through S66 + 4 S68 vacation-settlement mappers per ADR-033 slice 1a + 3 S70 termination-foundation mappers per ADR-033 slice 3a + 3 S71 slice-3b mappers — `SettlementReversed` graduated from define-only to emitted in S71; the 3 remaining DEFINE-ONLY settlement events — `FeriehindringTransferred`/`FeriehindringPaidOut`/`SaerligeFeriedagePaidOut` — are EventSerializer-registered for replay but have NO mapper/catalog row until their automation slice (4); the PARKED §7 `TerminationModregningApplied` is NOT defined at all — its payload shape awaits the SLS dialogue, SPRINT-71 gate (i)).
