...
3. **`TBD-defined-but-unemitted`** (0 rows — RESOLVED in S90) — This marker previously held `PayrollExportGenerated`, the vestigial S22-era class registered in EventSerializer with zero production emit sites. **S90 (ADR-034 / TASK-9001) resolved it via option (a):** the event class was RESHAPED into the per-(employee, year, month) payroll-export lock fact, given an Infrastructure `IAuditProjectionMapper<PayrollExportGenerated>` + a Payroll-`Program.cs` DI registration, and promoted to an `interface (cross-process)` catalog row. Emission lands in TASK-9002. No rows carry this marker anymore.
...
| `OrganizationDeleted` | TENANT_TARGETED | the org itself | `org_id` | `{orgId, orgName, orgType}` (S98 ADR-035 — GlobalAdmin org soft-delete, blocked-if-employees; `is_active=FALSE` flip on `org-{orgId}`) | interface | S98 |
| `OrganizationMoved` | TENANT_TARGETED | the org itself | `org_id` | `{orgId, oldParentOrgId, newParentOrgId, oldMaterializedPath, newMaterializedPath}` (S98 ADR-035 — GlobalAdmin ORGANISATION re-parent + the moved row's materialized_path rewrite; old+new parent+path carried for replay; on `org-{orgId}`) | interface | S98 |
...
| `PayrollExportGenerated` | TENANT_TARGETED | `employee → users.primary_org_id` | `employee_id` | `{action (PAYROLL_EXPORTED), employeeId, year, month, exportId, contentHash, exportedAt, periodId}` (RESHAPED in S90 from the vestigial S22 trace event into the per-(employee, year, month) payroll-export lock fact — ADR-034; the `payroll_export_records` row commits == sent to payroll; emitted per employee-month on `employee-{id}` from TASK-9002) | interface (cross-process — mapper in Infrastructure, not Backend.Api) | S90 |
...
| `EmployeeProfileScheduledChangeRetired` | TENANT_TARGETED | `employee → users.primary_org_id` | `employee_id` | `{profileId, employeeId, scheduledFrom, retiredBy, …}` — emitted once per scheduled row destroyed by a profile DELETE (owner ruling OQ-5 (a): deleting a profile also retires any scheduled change, **and the retirement must itself be audited** — a row audited into existence must not vanish unrecorded). Distinguished from the delete's own row by a `source` discriminator in the JSONB, because both carry `action = DELETED` and the column is CHECK-constrained to four values (S141 is schema-free) | interface | S141 |
...
| `BackdateWorklistRowCreated` | TENANT_TARGETED | `employee → users.primary_org_id` | `employee_id` | `{worklistId, employeeId, kind (EXPORTED_MONTH/SETTLED_YEAR), year?, month?, exportId?, entitlementType?, entitlementYear?, trigger {kind, eventId, effectiveFrom, actorId, baseline…}, appended}` | interface (S138 TASK-13803 — `BackdateWorklistRowCreatedAuditMapper`; one row per worklist row created OR appended, in the correcting write's tx) | S138 (close commit) |
| `BackdateWorklistRowResolved` | TENANT_TARGETED | `employee → users.primary_org_id` | `employee_id` | `{worklistId, employeeId, kind, resolution (RECALCULATED/DISMISSED), reason, versionBefore, versionAfter}` | interface (S138 TASK-13803 — `BackdateWorklistRowResolvedAuditMapper`; the Resolved event + this row ARE the audit record — no `*_audit` table) | S138 (close commit) |

**Total**: 78 rows (counts refreshed 2026-08-13; +7 S103 Unit-family mappers per ADR-038 are in the table but not itemised in this legacy tally: 54 through S66 + 4 S68 vacation-settlement mappers per ADR-033 slice 1a + 3 S70 termination-foundation mappers per ADR-033 slice 3a + 3 S71 slice-3b mappers — `SettlementReversed` graduated from define-only to emitted in S71 — + 2 S74 manager-vikar mappers per ADR-027 Phase 5 + 1 S79 slice-4 mapper (`FeriehindringTransferred` — graduated from define-only to emitted by the §22 FERIEHINDRING resolve disposition) + 1 S80 slice-2 mapper (`SaerligeFeriedagePaidOut` — graduated from define-only to emitted by the SPECIAL_HOLIDAY §15 stk.2/§17 godtgørelse close) + 1 S90 mapper (`PayrollExportGenerated` — reshaped from the vestigial S22 `TBD-defined-but-unemitted` trace event into the ADR-034 per-(employee, year, month) payroll-export lock fact; emission lands in TASK-9002) + 2 S98 mappers (`OrganizationDeleted` / `OrganizationMoved` — the ADR-035 GlobalAdmin org-structure ops, both emitted from the AdminEndpoints delete/move tx); the 1 remaining DEFINE-ONLY settlement event — `FeriehindringPaidOut` (§25) — is EventSerializer-registered for replay but has NO mapper/catalog row until its automation slice; the PARKED §7 `TerminationModregningApplied` is NOT defined at all — its payload shape awaits the SLS dialogue, SPRINT-71 gate (i)).

## Catalog closure status (S44c)

**74 of 78 rows have `mapper_kind = interface`** (count refreshed 2026-08-13; the "S44c" header is the frozen origin scope) — all mappers shipped across S44/S44b/S44c/S45; `PayrollExportGenerated` joined in S90; the S103 Unit family and later slices since. 4 rows remain TBD/define-only.

**4 rows remain deferred with TBD-* markers:**
- 0× `TBD-defined-but-unemitted` (PayrollExportGenerated — RESOLVED in S90: reshaped into the ADR-034 payroll-export lock fact + given an Infrastructure cross-process mapper + a Payroll DI registration; promoted to an interface row. Emission lands in TASK-9002.)
...
- `OrganizationDeleted` — RESOLVED at S98 (ADR-035 — the GlobalAdmin org soft-delete event class + mapper now exist; emitted from `DELETE /api/admin/organizations/{orgId}`). `OrganizationMoved` (re-parent) added alongside.
- `UserDeleted` — no event class exists (users are soft-deactivated via `employment_end_date`, not deleted).
