...
in the counts — Reviewer APPROVED-WITH-WARNINGS 0B/1W/4N; details in `SPRINT-139.md` § Review). Awaiting the owner's ruling (OQ-2 (b): shape + aging/escalation for the DECISION-READY rows only). Row ids
...
| **Decision-ready** | **1** (HRP-010) · **NOT READY 14** — all fourteen lack a stated deadline source; HRP-014(b) additionally lacks a named role |
...
## Owner ruling (S139 close)

_To be recorded in `SPRINT-139.md` and mirrored here: the shape (recommendation: 3); the HRP-010 aging proposal
with its projection caveat; and any missing-FACT rulings the owner chooses to make now (the institutional payroll
cutoff would settle HRP-011/012/022 at once) — those rows would then become READY and receive proposals in a later
cycle. NOT READY rows without a ruling are routed to Phase B with the missing fact._
