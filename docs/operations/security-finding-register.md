...
## Group 10 — discovered during S132 remediation (registered 2026-08-24 at S133 open)

Three security-flavoured follow-ups surfaced by the S132 review lenses (not in the S131 sweep). Registered so
none is lost; enumerated in `SPRINT-132.md` §S132-discovered follow-ups. **S133 does NOT fix these** (S133 is
test integrity) — they are registered + routed. Severity here ranks engineering priority, not live incident
risk (learning-project framing — no real users/data): all three are hardening/craft items on the design
target's road to production-readiness.

| SEC | What it means (plain language) | Title | Origin | Sev | OWASP/STRIDE | Status | Source of truth | Adjudication |
|-----|--------------------------------|-------|--------|-----|--------------|--------|-----------------|-------------|
| SEC-043 | The DB-auth path returns immediately for an UNKNOWN username (before running the password hash) but runs the BCrypt hash for a KNOWN username with a wrong password — the response-time difference lets a caller tell which usernames exist (a user-enumeration oracle). SEC-040 closed the logging/observability half; this timing half is separate. | Login response-timing user-enumeration oracle | S132-discovered (SEC-040 sibling) | Low | A07 / Info-disclosure (enumeration) | REGISTERED (S132-discovered); deferred hardening | `AuthEndpoints.cs` (DB-auth path, pre-BCrypt early return) | `SPRINT-132.md` §S132-discovered #5. Fix candidate = constant-time path (dummy hash on the unknown-user branch). |
