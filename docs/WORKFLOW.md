...
7a. **External Review (Sprint-End)**: After tests pass, internal Reviewer findings are resolved, and any worktree merges (step 6) are complete, the Orchestrator invokes Codex on the full sprint diff: `codex review --base <sprint-start-commit> "<prompt>"`. See [AGENTS.md](AGENTS.md) External Review section for the prompt template. `<sprint-start-commit>` is the HEAD commit of the previous sprint on master. Codex findings are recorded in the sprint log under "## External Review (Step 7a)" and mapped to BLOCKER / WARNING / NOTE severities. BLOCKERs should be fixed as follow-up tasks within the same sprint; re-invoke Codex after fixes. **Cycle cap**: if Codex has been invoked twice within this sprint's Step 7a and is about to be invoked a third time, the Orchestrator MUST halt and prompt the user to choose: (a) continue iterating, (b) accept remaining findings and commit, or (c) defer findings as a new sprint task. The sprint is "validated, not sealed" until Step 7a passes or the user grants a documented exit. If the `codex` CLI is unavailable, halt and prompt the user — do not silently skip.
