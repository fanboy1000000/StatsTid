...
it for a floor review. `general-purpose` has editing tools and `reviewer`'s tool list does not (its `Bash` could still write, so this is an instruction, not a guarantee, for both), so tell the arm it is read-only and record a content baseline before it starts — `git diff HEAD > pre.patch` plus a checksum list of untracked files (`git ls-files --others --exclude-standard | xargs sha256sum`) — and compare both after it returns. Identical `git status` output alone proves nothing: a file the sprint had already modified can be modified again without its status changing. Keep the comparison arm's output **out of the Step-7a reviewer artifact** under
