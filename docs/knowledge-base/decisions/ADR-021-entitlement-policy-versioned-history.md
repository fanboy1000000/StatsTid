...
## S81 Amendment — dated entitlement-config resolution sweep (D4 refinement)

S81 (the dated entitlement-config resolution sweep; priority #4 version-correctness) corrects how the **D4 two-step consumption pattern** was realized at the consumption sites and completes the dated-config intent across all read/validation surfaces.

**The defect.** D4's Step-2 ("dated read at the entitlement-year-start") was implemented at three sites — the Skema quota validation (`SkemaEndpoints.cs` quota loop), the Balance `/summary`, and the Balance `/series` — with a **misaligned anchor triple**: the dated *row date* was the year-start, but the **OK version was live (today)** and the **agreement was month-start-dated**. The newer YearOverview path (S65) used the fully year-start-aligned triple. So for an employee whose OK version OR agreement differed between the entitlement-year-start and the operation date, quota **validation** (and the balance **summary**) resolved `annual_quota`/`carryover_max` against different numbers than the **year-overview/accrual** displayed — a validation-vs-display split-brain.

**The correction.**
1. **Hoisted resolver.** A `DatedEntitlementConfigResolver` (Backend/Infrastructure — it does repository I/O, unlike the pure SharedKernel `EntitlementPeriodResolver`; per-request factory) encapsulates the dated-agreement → dated-OK → **graceful** live-fallback chain (ADR-023 D3, "never 500"). All **four** consumers — Skema quota validation, `/summary`, `/series`, and YearOverview — now resolve the Step-2 quota at the **year-start-dated OK version AND year-start-dated agreement** through this single resolver.
2. **D4 §"frozen `reset_month`" refinement.** D4 stated the live `ResetMonth` is "safe to use for year-start derivation across the full history." That holds **only within a single natural key** `(type, agreement, ok)` (the D5 freeze) — NOT across an OK/agreement change. The normal admin create path can mint a fresh key with a different `reset_month` for the unconstrained IMMEDIATE types (only VACATION is schema-pinned to `reset_month=9` via the DB CHECK + create-guard; SPECIAL_HOLIDAY is `reset_month`-independent, 1-Jan-anchored per S80/ADR-033). So S81 **re-derives the consumed entitlement year from the OPERATION-DATE key's `reset_month`** (op-date OK + op-date agreement), correct-by-construction even when `reset_month` diverges across keys — Step-1 reads the operation-date key, then Step-2 reads the quota at the resulting year-start. This applies to **all four consumers, including YearOverview's per-month `saldo`/`afholdt` AND its closed-ferieår disposition (`expiring`/`closedEntYear`)** — the latter anchors the closed-year `reset_month` at Jan-1 of the selected calendar year (within the ferieår that closes during the year for any `reset_month`, so non-circular).
3. **Graceful-vs-fail-closed split preserved.** The shared resolver is the **graceful** family only (ADR-023 D3). The `VacationSettlementService` dated-config resolvers stay **fail-closed** (ADR-033 D10 — a leaver/termination settlement must never value against the live quota); they are NOT routed through the shared resolver. A regression pins this isolation.

**Invariant added.** All quota-config reads at the four consumption/display surfaces resolve `(type, year-start-dated agreement, year-start-dated OK, year-start row-date)`; the consumed entitlement year is derived from the operation-date key. Validation == display for any OK/agreement-history-changed employee. Durable detail: `docs/sprints/SPRINT-81.md` + `.claude/refinements/REFINEMENT-s81-dated-okversion-config-sweep.md`.
