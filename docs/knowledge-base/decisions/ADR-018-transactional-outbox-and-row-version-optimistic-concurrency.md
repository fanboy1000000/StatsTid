...
| `IEventStore` in-tx overload | In-tx `EnqueueAsync` writes outbox row in caller's tx; rollback removes outbox row; ETag/version backfill on existing profiles returns `1` | 3 | `tests/StatsTid.Tests.Regression/Outbox/EventStoreInTxTests.cs` (+ `tests/StatsTid.Tests.Unit/Outbox/EventStoreInterfaceTests.cs` if needed for unit coverage — never created; the Regression test carries the coverage) |
...
### D14 — Wage-type-mapping versioned history uses surrogate UUID PK + effective-dating + export-time effective-date lookup (S29)

Added in S29 / TASK-2910 as an additive amendment within the already-accepted ADR family (no Status bump per S27 refinement cycle 3 Codex N1 precedent). D14 documents the WTM-specific application of S21 / ADR-017 D2.1's effective-dating pattern, together with the export-time effective-date lookup that closes the ADR-016 D10 replay-determinism gap for `wage_type_mappings`.

**Decision**: `wage_type_mappings` follows the S21 `local_agreement_profiles` effective-dating pattern (surrogate `mapping_id UUID` PK + natural-key partial-unique-index on open rows + same-day-vs-cross-day routing + 3-case CREATE routing per ADR-020 D2). Replay-deterministic lookup is achieved via an `asOfDate` parameter on `PayrollMappingService.GetMappingAsync`, which routes to a dated read (`WageTypeMappingRepository.GetByKeyAtAsync`) that replicates the existing `(position = @position OR position = '')` position-fallback at the dated predicate. ADR-016 D10 is satisfied at the export boundary, NOT at the planner boundary — explicitly DIFFERENT from D11's "manifest bypasses live DB" mechanism, but producing equivalent determinism for this consumer shape.

**Rationale**: WTM is consumed only at the export boundary (`PCS.MapSegmentToExportLinesAsync`). A planner-time snapshot of the resolved `mapping_id` adds no value when the consumer can do a deterministic dated read using a replay-stable natural-key triple. The triple is carried through `SegmentSnapshot.Values["WtmNaturalKey"]` via the ADR-020 D1 planner-enrollment seam, so the segment manifest persists the inputs needed to re-resolve the row at replay time — not the resolved row itself. This is the fourth pattern in ADR-016 D5b's framework (export-time effective-date lookup); see ADR-016 D5b reconciliation.

**Consequences**:
- WTM CREATE-side races resolved via `SELECT ... FOR UPDATE WHERE effective_to = today AND natural_key = ...` + ADR-020 D2 3-case routing inside a single transaction. Lock held until commit. Case A (no row) relies on the partial-unique-index `idx_wtm_natural_key_open WHERE effective_to IS NULL` as the race-safety mechanism (mirrors S22 `LocalAgreementProfileRepository.cs:317-336` empty-slot precedent); 23505 from the index translates to 412/409 at the endpoint layer.
- WTM PUT routes through `WageTypeMappingRepository.SupersedeAndCreateAsync(conn, tx, ...)`; same-day edit (predecessor `effective_from == today`) → in-place UPDATE + `UPDATED` audit + `WageTypeMappingUpdated` outbox event; cross-day edit (predecessor `effective_from < today`) → close predecessor + INSERT new row + `SUPERSEDED` audit + `WageTypeMappingSuperseded` outbox event.
- WTM DELETE becomes soft via `SoftDeleteAsync(conn, tx, ...)` setting `effective_to = today`. `WageTypeMappingDeleted` event payload semantic changes from "mapping is gone" to "mapping closed at today" — replay re-reading via `GetByKeyAtAsync(date)` returns the closed row for `date < closure` (with full payload, including `wage_type` — load-bearing for replay-determinism) and NULL for `date >= closure` (existing log-and-skip behavior at PCS L1180-1184 handles).
- Endpoint same-day-only-edit validator: POST + PUT reject `body.EffectiveFrom != today` with 422 BEFORE opening the tx. Symmetric forbid — both backdated AND future-dated edits rejected to preserve replay determinism (backdated would retroactively insert history; future-dated would create `effective_to < effective_from` invariant violations). DELETE handler is unaffected — `effective_to = today` is always today, no caller-supplied effective_from.
- `MapCalculationResultAsync` (per-line-date export path) stays current-row per ADR-020 Implications §7 + ROADMAP "no retroactive recomputation" — explicit non-change. `MapSegmentToExportLinesAsync` (per-segment export) reads natural-key triple from `Snapshot.Values["WtmNaturalKey"]` and passes `asOfDate=segment.StartDate` to `GetMappingAsync` (which routes to `GetByKeyAtAsync`).
- Snapshot round-trip: `Snapshot.Values` is `IReadOnlyDictionary<string, object?>` and serializes to `segments_jsonb` via System.Text.Json. On replay, the typed `WtmNaturalKey` record materializes as `JsonElement` (the values-dict's `object?` type doesn't preserve runtime type info). Consumer at `PCS.MapSegmentToExportLinesAsync` MUST handle both: forward path receives `WtmNaturalKey` directly; replay path receives `JsonElement` and deserializes with the same `JsonOptions` used for write. This is a localized concern for any non-primitive typed snapshot value — Phase 4d-2/4d-3 may revisit if more typed snapshots are added (typed-discriminator scheme).
- No conflict with D13 sync-in-tx projection pattern — D13 governs event-stream-backed projections (Skema / Time / Balance / Compliance per S27). WTM is its own table-backed source — reads against `wage_type_mappings` ARE read-your-write since the table is the canonical source. The two patterns coexist; D14 documents WTM's distinct mechanism for closing ADR-016 D10 via export-time effective-date predicate.

**Test floor**: 12 D-tests under `tests/StatsTid.Tests.Regression/Config/` (same-day UPDATE, cross-day supersession, `GetByKeyAtAsync` correctness, D2 Case B + Case C routing, ETag/If-Match contract, backfill idempotency, partial-unique-index race, seed-INSERT idempotency under accumulated history, concurrent DELETE↔POST race, concurrent POST↔POST race on empty natural key, same-day-only-edit validator past/future/today) + 1 marquee D-test extending `tests/StatsTid.Tests.Regression/Segmentation/ReplayDeterminismTests.cs` (`ReplayAsync_StableUnderWtmMutation_ExportLinesByteIdentical` — closes the deferred-WTM caveat at L14-23 with byte-identical assertions on hours sum + per-line wage-type column). Plus 1 unit test for `PeriodPlanner.Plan(enrollment: null, profile: null)` direct-planner backward-compat.

**Open** (deferred to Phase 4d-2 / Phase 4d-3 / Phase 4e):
- Future-dated PUT support — S29 forbids future-dated edits per the symmetric validator; Phase 4d-2 may revisit when entitlement_configs / employees join the versioned-history family.
- `BoundaryCause.WageTypeMappingChange` planner enrollment — deferred per ADR-020 Open Q6; the current pattern reads `wage_type_mappings` only at the export boundary, NOT at the planner boundary, so no `BoundaryCause` entry is needed for S29.
- Clock abstraction — endpoint reads `DateOnly.FromDateTime(DateTime.UtcNow.Date)` directly (mirrors S22 `LocalAgreementProfileRepository.cs:584` precedent); Phase 4e candidate per ADR-020 Open §clock-abstraction.
- Frontend history view UX — deferred to Phase 5 per ADR-020 Open §Phase-4d-1-frontend-view-history.
...
### Cycle 8 (2026-05-11) — D14 added (S29 / TASK-2910)

Additive amendment within the already-accepted ADR family (no Status bump per S27 cycle 7 precedent — same justification carried forward). D14 documents the wage-type-mapping versioned-history pattern shipped in S29 against ADR-020's binding contract: surrogate UUID PK + effective-dating columns + partial-unique-index on open rows + same-day-vs-cross-day routing in the repo + ADR-020 D2 3-case CREATE routing in the endpoint, plus export-time effective-date lookup at `PCS.MapSegmentToExportLinesAsync` consuming the natural-key triple from the planner-enrolled `Snapshot.Values["WtmNaturalKey"]`.

D14 explicitly differentiates from D13: D13 governs event-stream-backed projections (Skema / Time / Balance / Compliance per S27); D14 governs table-backed sources whose reads are inherently read-your-write because the table IS the canonical source. The two patterns coexist; no consolidation is warranted.

D14's "Snapshot round-trip" consequence pins the JsonElement-on-replay handling at `PCS.MapSegmentToExportLinesAsync` — a localized concern that fires for any non-primitive typed value flowing through `Snapshot.Values`. S29 is the first sprint to add such a value (WtmNaturalKey record); Phase 4d-2 / 4d-3 may revisit with a typed-discriminator scheme if more typed snapshots are added.

S29's TASK-2912 fix commits — `ba2ad29` (init.sql bake S29 schema into CREATE TABLE), `c332216` (complete partial TASK-2906 seed rewrite), `3606e06` (JsonElement round-trip at PCS WtmNaturalKey consumption) — were caught by TASK-2909 D-tests and absorbed inline. The marquee D-test `ReplayAsync_StableUnderWtmMutation_ExportLinesByteIdentical` is the load-bearing proof: closes the ADR-016 D10 violation captured in `tests/StatsTid.Tests.Regression/Segmentation/ReplayDeterminismTests.cs:14-23` (the pre-S29 deferred-WTM caveat) with byte-identical assertions on hours sum + per-line wage-type column.
