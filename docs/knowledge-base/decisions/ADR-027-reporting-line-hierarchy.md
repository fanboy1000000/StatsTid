...
> **Reshaped by [ADR-035](ADR-035-flat-authority-model.md) (S92→S95, flat authority):** the multi-tree / styrelse boundary this ADR is framed around (`tree_root_org_id`, same-tree checks, the D13 within-the-styrelse authority) was retired at S95 — authority checks are now a flat exact-Organisation set. ADR-035 D6 carries the per-decision disposition table (kept / reshaped / retired) for this ADR's D2/D9/D11/D13/D15/D18/D19; the "styrelse" framing below is historical.
>
...
- **Inclusive "til og med" fix:** `DelegationExpiryService` closes a vikar the day AFTER `until_date` (predicate `until_date < `), so the named day is the LAST covered day. *[Superseded twice and annotated at S142 close: the predicate read `CURRENT_DATE` — the DATABASE server's clock — until S140 bound it as an application-supplied `@today`; ADR-041 then made that day the **Europe/Copenhagen** calendar day, not the UTC one. The rule itself (close the day AFTER `until_date`) is unchanged. Annotated rather than rewritten because the ADR is a dated decision record, but a predicate quoted in the present tense reads as current mechanism.]*
...
- **New `GET /api/admin/reporting-lines/{managerId}/vikar` (S76b, additive read):** a companion read for the unified-editor FE — when the EditPersonDrawer is opened from the UserManagement *list* (no tree context to supply `activeVikar`), the drawer self-resolves the manager's active vikar from this read so an away-manager's vikar shows + is revocable from that entry point. **LocalAdmin, admin-role-floored** scope check on the manager's org (the same B1 floor as the create path); returns `{ activeVikar | null }` (the stand-in's id + display name via `ManagerVikarRepository.GetActiveByApproverWithVikarNameAsync`, a `LEFT JOIN` on `users.display_name`); 404 on an unknown manager. Read-only, no lock (a torn read self-heals on the next mutation; the tree context still wins when present). Caught at S76b Step-7a — the external lens found the from-list vikar-hydration gap the internal structural trace missed ([[review-lens-complementarity]] on the FE-contract layer).

### Program status — Medarbejder administration (ADR-027 Phase 5) FEATURE-COMPLETE (S77 / 2026-06-15)

The 4-phase program (S74 backend foundations · S75 read-mostly tree · S76 write-flow backend · S76b unified-editor FE · S77 polish + close) is **FEATURE-COMPLETE**, with the validation pinned by S77: the **UI-driven approver-through-vikar cross-flow** (`ApprovalDashboard`/Godkend tid, a Leader-level vikar approving/rejecting — edge-only authority) + the **composed backend vikar approve→reopen→reject** lifecycle (the legal two-period path) + a bounded **2000-employee** roster/search scale validation; the dead `ReportingLineTree` FE was retired (the resolver/endpoints stay).

- **Reopen-gate posture (S77, INTENDED — documented).** The FE shows the `Genåbn` (reopen) button only to `LocalHR+` (`ApprovalDashboard.tsx:132`), even though the backend designated-edge **does** grant reopen authority to a Leader-level designated approver/vikar (the reopen-Leader-arm). This is a **deliberate** stricter-in-the-UI posture (reopen / un-approve is HR-only in the surface), NOT a bug: the capability exists server-side (and is regression-proven via the backend test), but is intentionally not exposed to a non-HR approver in the dashboard. Recorded so a future reader does not "fix" the gate.
- **Open security-hardening residual (NAMED, deferred — owner-ruled).** The program is feature-complete but NOT security-complete: the **approval-auth TOCTOU** remains open — the authorization on approve/reject/reopen (`ApprovalEndpoints.cs:205/351/961`) is checked OUTSIDE the action transaction; the **designated-EDGE authorization half was introduced by THIS program (D13)** while the org-scope half is pre-existing. Plus the cross-styrelse-transfer serialization (D15, the advisory key from mutable org data). Both are NON-corrupting (documented in D13/D15) and deferred to a dedicated **in-lock authorization-hardening** sprint; closing them would lift Reporting-Line & Approval Routing from A− to A. **→ The dedicated sprint is S78 (D18 below); it closes the cross-transfer drift in full + the edge-auth race for the key-sharing vectors + the status race, holding A− with a precise NAMED residual map.**

### D18 — In-lock authorization hardening (S78 — RESOLVES the D15 cross-transfer residual; PARTIALLY resolves the D13 approval-auth TOCTOU)

S78 (owner rulings: OQ-1a edge-auth = CONTAINED; OQ-DRIFT = SOUND) closes the deferred concurrency residuals to a precise, honest scope. Backend-only; NO schema/event change.

- **The cross-styrelse-transfer stale-key drift (the D15/S74-7403 residual) is FULLY CLOSED.** A shared **drift-guarded acquire** (`ReportingLineRepository.AcquireTreeLockForEmployeeAsync` + the transfer variant) derives the employee's current tree root, acquires the `reporting-tree-{root}` xact advisory, RE-DERIVES under the held lock, and on drift (a transfer committed mid-acquire) **rolls back + retries the whole request** on a fresh READ COMMITTED tx (bounded ≤3 → a pinned 409; NOT a mid-tx release — `pg_advisory_xact_lock` releases only at tx end; NOT session locks). Applied to EVERY employee-current-root tree mutator: reporting-line assign/remove/acting, atomic-create-delete-with-reassignment, the self-`/delegate` CREATE, the admin-vikar CREATE, the approve/reject/reopen-Leader paths, AND the live cross-styrelse transfer mutator (`PUT /api/admin/users/{userId}` changing `primary_org_id`, which now takes the OLD+NEW tree advisories advisory-before-the-users-`FOR UPDATE`). All concurrent pairs now provably hold the SAME current key → genuine mutual exclusion under a simultaneous transfer.
- **The approval status double-write race is closed (both arms).** The status transition is a CONDITIONAL `UPDATE … WHERE period_id AND status IN (<allowed>)` (with `FROM (SELECT status … FOR UPDATE) RETURNING` the locked-in old status — so the audit/event `PreviousStatus` is the actual locked status, not a stale pre-tx read), `0 rows → 409`, as the FIRST mutation before any FallbackTraversalWarning/audit/outbox — so the loser writes nothing. Applied to approve/reject/reopen-Leader/reopen-Employee (the employee arm gets the conditional UPDATE but not the edge-auth lock).
- **The approval-auth EDGE race is closed for the KEY-SHARING vectors.** approve/reject/reopen-Leader acquire the `reporting-tree` advisory (drift-guarded) as the first lock-bearing statement, then RE-EVALUATE the designated-edge authorization AND re-derive the enforcement classification (the REQUIRED-mode `confirmFallback` gate + the persisted `designated_approver_id`/`approval_method`) STRICTLY under the held advisory — because the action tx holds the advisory, any concurrent key-sharing revoke (reporting-line remove, admin-vikar CREATE, self-`/delegate` CREATE) blocks before its commit, so the in-tx re-read observes the frozen state (a revoke committed after the pre-tx check is caught → 403; a reassignment is re-classified → 428). The org-scope half is NOT re-checked in-tx (it is JWT-claim-based — a DB lock cannot serialize a bearer-token grant).
- **NAMED RESIDUAL MAP (NOT closed by S78 — owner-ruled contained):** the edge-auth race remains open for the revoke vectors that do NOT share the `reporting-tree` key — the **self-service `DELETE /delegate`** (REPEATABLE READ, persisted/own path), **role-assignment deactivation**, **user deactivation** (`users.is_active`, flipped by ~4 paths incl. the settlement leaver flip — intractable to fully serialize), and the **JWT-scope revocation** (token-lifetime, not DB-lockable) — PLUS the **admin-vikar-REVOKE post-transfer key-mismatch** (the revoke deliberately keys on the PERSISTED `manager_vikar.tree_root_org_id` for revoke-safety, so after a transfer its key can differ from the employee-current root). All NON-corrupting + documented (see `docs/SECURITY.md`). **Reporting-Line & Approval Routing HELD A−** — the cross-transfer drift is fully closed, but these edge-auth revocation residuals keep it below A. **→ The "full" pass is S83 (D19 below): it closes R1 (self-`DELETE /delegate`) + R2 (the admin-revoke post-transfer mismatch), rules R3 (role-deactivation) ACCEPTED, and accepts R4 (user-deactivation) + R5 (JWT) as named platform residuals — holding a tightened A−.**

### D19 — Full edge-auth serialization pass (S83 — closes the tractable S78 residual subset; accepts the rest)

S83 (owner rulings 2026-06-19: OQ-1 ACCEPT R3 as policy; OQ-2 ACCEPT R4+R5 as platform residuals; OQ-3 grade = tightened A−, NOT flat A) closes the tractable part of the D18 residual map and formally accepts the intractable part. Backend + docs ONLY; NO schema/event/FE change.

- **R1 + R2 CLOSED via ONE shared helper `ReportingLineRepository.AcquireRevokeTreeLocksAsync(conn, tx, persistedRoot, subjectId, ct)`.** It locks the persisted root ALWAYS (the immutable revoke-authority anchor — survives an inactive/transferred subject), then **defensively derives** the subject's employee-current root (`DeriveEmployeeTreeRootInTxAsync` inside a `try`/`catch (InvalidOperationException)` — no separate `is_active` pre-gate; catching the derive's throw is what closes the active→inactive race so a mid-request deactivation never 500s a revoke-safe path), acquires `{persisted} ∪ {current?}` in **distinct + id-sorted order** (the `AcquireTreeLocksForTransferAsync` idiom — uniform ordering, deadlock-safe vs the transfer path; all advisories before any row `FOR UPDATE`), and **drift-guards the current root only** (the persisted root is a fixed column and cannot drift; a deactivation under the lock falls back to persisted-only, no error).
  - **R1 — self-`DELETE /api/reporting-lines/delegate`:** was `RepeatableRead` with NO advisory; now `ReadCommitted` under `TreeRootDriftRetry.RunAsync` with a pre-lock persisted-root probe (`GetActiveByApproverAnyDateAsync(actorId)`) → the helper. (R1 originally scoped single-key; the S83 Step-0b external lens proved that unsound — `CloseByApproverAsync` keys on `absent_approver_id` while the row carries a persisted `TreeRootOrgId`, so a self-delegation whose owner transferred AFTER creating it has persisted≠current → R1 mirrors R2; unified onto the shared helper.)
  - **R2 — admin `DELETE /api/admin/reporting-lines/{managerId}/vikar`:** already had the persisted-root advisory + in-lock FOR UPDATE/authorize/close; S83 ADDS the current-root advisory (via the helper, wrapped in `TreeRootDriftRetry.RunAsync`) so the revoke serializes against a concurrent transfer-into-a-new-tree. The authorize still anchors on the pinned row's persisted `TreeRootOrgId`, and the emitted `ManagerVikarEnded.TreeRootOrgId` still comes from the pinned row.
  - **Named residual:** when the manager is *inactive*, the current root is not derivable → persisted-only fallback — non-corrupting (the only current≠persisted divergence is a transfer, which requires an *active* employee, so the inactive-during-transfer corner is near-impossible; the revoke only ENDS an edge).
- **R3 — role-assignment deactivation: ACCEPTED as non-corrupting policy (NOT closed).** A real non-serialized window exists (the approve/reject in-lock re-check reads role state on its own connection with no RBAC lock — `src/Infrastructure/StatsTid.Infrastructure/DesignatedApproverAuthorizer.cs:131-145`; role-revoke takes no advisory — `AdminEndpoints.cs:1804/1810`), so a role-revoke can commit between the re-check and the approval commit. Deliberately NOT dragged into the per-employee tree lock — RBAC scope is a separate bounded context (priority #1); the blast radius is a stale-authority approval, reversible via operator-initiated `reopen` (not auto-healed), fully audit-traceable. The deactivation arm (`users.is_active=FALSE`) IS covered (the in-lock `active+LeaderOrAbove` predicate fails closed — pinned by test, S83/TASK-8303).
- **R4 (user-deactivation) + R5 (JWT-scope) ACCEPTED as named platform residuals.** R4 `users.is_active` is written by 3 paths across two lock domains (`ReportingLineEndpoints.cs:1484` tree-lock; `UserRepository.cs:445` employee-lock; `SettlementCloseService.cs:517` background employee-lock) — this enumerated count SUPERSEDES the D18 "~4 paths" estimate; full serialization would require tree-locking a background service across lock domains (platform scope). R5: scopes are minted into the JWT at login (`JwtSettings.cs:8`, 480-min TTL) with no runtime invalidation — a true fix needs short token TTL / a revocation list (platform feature).
- **Grade:** owner OQ-3 — **tightened A−**, NOT flat A, while R3 (accepted-not-closed) + R4/R5 (platform) residuals stand (the S77 over-claim guard). Process: the Step-0b external lens caught TWO BLOCKERs the internal Reviewer rated sound (the R1 owner-transfer divergence + the active→inactive 500 race) — fixed pre-code, cycle-2 verified clean ([[review-lens-complementarity]]).
