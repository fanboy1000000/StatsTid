...
**Transactional contract (cycle 1 review resolution; cycle 2 reverted, see footnote):** the goal is that the `LocalAgreementProfileChanged` event-store append, the audit-projection insert (D8), and the close-then-insert UPDATE+INSERT on `local_agreement_profiles` all participate in one atomic write so that a partial failure can never leave the audit committed without the event (or vice versa).

If a future architectural change moves the event store or audit projection to a different connection / different database, an `AuditState`-style outcome model (ADR-016 D10) becomes necessary. Document explicitly so the simplification doesn't quietly drift.

**Cycle-2 review footnote (S21 known limitation):** Cycle 1 implemented this contract by passing the caller's `NpgsqlConnection` + `NpgsqlTransaction` into a new `IEventStore.AppendAsync(conn, tx, …)` overload, calling it before `tx.CommitAsync`. Cycle 2 found that under `RepeatableRead` (the isolation level the profile UPDATE+INSERT requires for `SELECT FOR UPDATE` ETag concurrency), the event-store's `SELECT MAX(stream_version) FROM events WHERE stream_id = …` reads from the caller's snapshot — taken BEFORE any concurrent commit on the same stream becomes visible. Two admins racing the same profile would therefore both compute the same `stream_version` and the second commit would fail with a `(stream_id, stream_version)` UNIQUE violation, rolling back the entire save. The original "self-contained `AppendAsync` after `tx.CommitAsync`" shape avoids this because each call gets a fresh transaction and snapshot.

**Decision (S21 cycle 2)**: revert to the post-commit "best-effort" shape for S21. The residual partial-failure risk (process crash between `tx.CommitAsync` and `eventStore.AppendAsync`) is rare for a pre-production single-instance API. Track full atomicity restoration in **Phase 4 — Transactional outbox for event-store + state-change atomicity**: an `outbox_events` table written inside the profile transaction; a separate publisher drains the outbox into the canonical event store; same atomic guarantee without MVCC snapshot conflicts. Sibling task to D2.2's ETag-pattern propagation — both are part of the same hardening pass.
...
One audit row per save. `delta_jsonb` mirrors the event's `ChangedFields` shape — same data, two persistence sites (event store + projection), identical semantics. The audit row write is in the **same DB transaction** as the profile UPDATE+INSERT (the event-store append happens after `tx.CommitAsync` per D6's cycle-2 footnote; full atomicity tracked for Phase-4 hardening).
...
- **D2 close-then-insert window math (Step-7a carry-forward, 2026-05-03):** Step-7a Codex review fixed predecessor-close-at-`newProfile.EffectiveFrom - 1` (was UTC `today`). The residual edge case — `newProfile.EffectiveFrom <= predecessor.EffectiveFrom` produces a predecessor row whose `effective_to` precedes its `effective_from` — needs a **row-version column for ETag** (sibling to D2.2's ETag/If-Match propagation work; UPDATE in place rather than close-then-insert when dates match) AND **end-exclusive `effective_to` semantics** (sibling to D6's transactional-outbox sub-sprint; eliminates the `-1` arithmetic at the boundary). Cycles 3-9 of Step-7a explored a temporal-monotonicity guard + UPDATE-in-place path; both reverted because each layered fix produced cascading regressions (lost-update on profile_id-as-ETag, audit-shape drift, self-cycle on `PrecedingProfileId`, response/GET divergence on `CreatedBy/CreatedAt`). Sized correctly for Phase-4, not for cycle-fix discipline. Cycle 10 verified the cycle-1-only diff clean.
