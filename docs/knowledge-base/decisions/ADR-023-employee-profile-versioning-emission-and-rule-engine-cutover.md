...
- **Real exposure**, NOT hypothetical: `AdminEndpoints.cs:466` PUT `/api/admin/users/{userId}` is scoped to `LocalAdminOrAbove` (any per-org admin), persists `agreement_code` directly at `AdminEndpoints.cs:512` with NO event emission, and `frontend/src/pages/admin/UserManagement.tsx:274` (S32 path — the page is now `OrganisationOgMedarbejdere.tsx`) actively sends `agreementCode: editForm.agreementCode` from a live form field. Any normal-flow admin edit during S32+ pre-launch makes past PCS replays for that employee non-stable.
