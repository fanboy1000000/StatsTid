...
| [ADR-036](decisions/ADR-036-structured-enhed-metadata.md) | Structured Enhed metadata + multi-tag membership — refines ADR-035 D2, replacing the free-text `employee_profiles.enhed_label` with two tables (the `enhed_label` column kept read-only/transitional). D1 `enheder` (per-Organisation; soft-delete; partial-unique `(organisation_id, lower(name)) WHERE deleted_at IS NULL`) + `user_enheder` (multi-tag junction; both FKs `ON DELETE CASCADE` — a junction owned by both parents; in prod neither is hard-deleted so the cascade only fires for hard-delete cleanups). An Enhed belongs to ONE ORGANISATION-typed org (create-endpoint guard). D2 managed-entities-first, flat/untyped, multi-tag; the same-Organisation invariant enforced at the COMMAND layer (set-tags validates each tag ∈ the FOR-UPDATE'd user's primary_org active enheder), NOT a DB trigger (transfer CLEARS tags in-tx). D3 ZERO authority — Enhed absent from `OrgScopeValidator`/`CoversOrg`/`DesignatedApproverAuthorizer`/`ValidateEmployeeAccessAsync`; the roster/search aggregation + `?enhedId` filter stay inside the users-first `accessibleOrgs` bound (keyed by enhed_id) → no cross-org leak. D4 event-sourced latest-wins projection (`EnhedCreated/Renamed/Deleted` + `UserEnhederChanged` full-set; NOT the ADR-022 temporal profile; `EnhedLabel` frozen on the EmployeeProfile* events). D5 endpoints `ValidateOrgAccessAsync(LocalHR)`-floored over the correct org (multi-org via `GetAccessibleOrgsAsync`); set-tags re-floors against the locked org (transfer-TOCTOU); rename/delete carry the If-Match version predicate INSIDE the UPDATE + affected-row check (optimistic concurrency); soft-delete + projection-filter (no fan-out). D6 `EnhedBackfillSeeder` reads the `enhed_label` projection column (DISTINCT per org; not event replay), idempotent, greenfield no-op. Step-7a: Codex caught 2 concurrency BLOCKERs (set-tags TOCTOU + rename/delete lost-update), both fixed. | accepted | S97 | Data Model, Infrastructure, Backend, Frontend | enhed, multi-tag, metadata, display-only, zero-authority, organisation, soft-delete, optimistic-concurrency, latest-wins-projection, event-sourcing, greenfield |
