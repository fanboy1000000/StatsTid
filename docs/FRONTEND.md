...
| `/admin/ledelseslinjer` | `MedarbejderAdministration` | LocalHR | Medarbejder administration — the structural approval tree + the unified `EditPersonDrawer` create/edit/approver/vikar/delete surface (ADR-027 Phase 5, S74–S77; replaced the retired `ReportingLineTree`). S91 (TASK-9103): opened to LocalHR — the old `UserManagement` list (`/admin/medarbejdere`) was removed and HR keeps employee management here (a deliberate P7 expansion; backend auth lowered in TASK-9102). **S107: to be REDIRECTED + retired into the merged page at the S108 cutover.** |
| `/admin/organisation-medarbejdere` | `OrganisationOgMedarbejdere` | LocalHR | **Enhedsspor merged admin (S107, Phase 3b-1 — the VIEW half).** The 3-region "Organisation & medarbejdere" page (`design_handoff_org_medarbejdere` Model A): left org-structure tree + recursive right "Struktur" (leaders→employees→child units) + Afgrænsning scope popover + search overlay — RENDERS structure+people from the S106 reads (`/api/admin/units/forest`, the unit-tagged roster, `/api/admin/search`), READ-ONLY (no mutation affordances — the S91 discipline; drawers/mutations + the cutover are S108). Runs ALONGSIDE the two old pages until the S108 cutover redirects `/admin/ledelseslinjer` + `/global/organisation` here. |
