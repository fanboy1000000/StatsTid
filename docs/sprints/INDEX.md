...
| [Sprint 33](SPRINT-33.md) | Phase 4d-3 Part 2 **IMPLEMENTATION**: ADR-023 D1-D8 cutover — implements binding contract authored in S32 design-only sprint. **13 tasks** across 4 phases (Phase 0 open + Phase 1 sequential foundation 4 tasks + Phase 2 parallel non-worktree cutovers 7 file-disjoint tasks + Phase 3 D-tests + Phase 4 close). New `EmploymentProfileResolver` service in Infrastructure + DI wiring in BOTH Backend.Api + Payroll.Integrations Program.cs (Codex W absorption). `EmployeeProfileRepository` gains `SupersedeAndCreateAsync` (ADR-020 D2 3-case routing under `SELECT ... FOR UPDATE`) + `SoftDeleteAsync` (predecessor version unchanged per ADR-023 D8; SQL no `version+1` clause; audit action `DELETED` matching `init.sql:514` CHECK constraint — refinement cycle 2 Codex BLOCKER absorption). PCS.cs:326-339 cutover with **trailing optional resolver** `IEmploymentProfileResolver? = null` (preserves S29-precedent test-fixture compatibility). `EmploymentProfile` refactored to `sealed record class` for `with` syntax. ComplianceEndpoints fail-closed (500 per ADR-023 D3 — replaces hardcoded `37.0m`); BalanceEndpoints graceful fallback (resolver → AgreementConfig → Central → 37.0m chain preserved). New DELETE endpoint at `/api/admin/employee-profiles/{employeeId}` (HROrAbove + OrgScopeValidator + ADR-019 admin-strict If-Match; emits `EmployeeProfileSoftDeleted`); PUT extended with required `EffectiveFrom: DateOnly` + cycle-3 validator rejecting both backdated AND future-dated with 422; cross-day emits `EmployeeProfileSuperseded`. AdminEndpoints PUT emits net-new `UserAgreementCodeChanged` event (55→56 typeof; `DateOnly EffectiveFrom`; null-safe Ordinal predicate; no S33 consumer = Phase 4e replay-data trail per ADR-023 D2). DEAD CODE removed: TimeEndpoints `/calculate` + `/calculate-week` + 2 contract DTOs + 2 tests per ADR-023 D6. Frontend `EmployeeProfileEditor` gains UX-only as-of-date toggle (read-only banner when `asOfDate !== today`) **+ PUT-body `effectiveFrom: today` injection** (refinement cycle 2 convergent BLOCKER absorption — keeps wire shape in sync with TASK-3308 backend DTO change). ~19 Docker-gated D-tests including 2-variant marquee `ReplayAsync_StableUnderEmployeeProfileMutation_ResultByteIdentical` (`weekly_norm_hours` + `part_time_fraction`; `position` covered by separate non-marquee D-test due to caller-fallback semantic). Refinement 3-cycle dual-lens at Step 4 (cycles 1+2 absorbed 5 BLOCKERs + 8 mechanical WARNINGs; cycle 3 verification-only clean — cycle-cap respected). At TASK-3313 sprint close: **ROADMAP Phase 4e `agreement_code` row upgraded from "candidate" to LAUNCH-BLOCKING** per ADR-023 D2 binding commitment. | in-progress | 2026-05-17 → — | — (target ~847: 526 unit + 35 plain + 198 Docker-gated + 88 frontend, +14 net new D-tests) | no |
