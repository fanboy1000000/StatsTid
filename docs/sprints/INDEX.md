...
| [Sprint 32](SPRINT-32.md) | Phase 4d-3 Part 2 **DESIGN-ONLY**: ADR-023 Authorship — produces ADR-023 "Employee-Profile Versioning Emission + Rule-Engine Cutover Architecture" settling 7 enumerated questions from the deferred S32-implementation refinement at `.claude/refinements/REFINEMENT-s32-phase-4d3-part2.md`. **No code changes; no test changes**. Mirrors S28 → S29 split precedent (third canonical thrash-defer case per `feedback_thrash_defer_real_world.md` after S28 = 1st and S31 cycle-2-converging-finite = 2nd control case). The S32-implementation refinement Step 4 cycle 1 found 2 convergent BLOCKERs (planner-enrollment seam shape mismatch + Q3 LEAVE not replay-safe for agreement_code); user picked option (a) "absorb in-refinement"; cycle 2 found 4 NEW BLOCKERs in SAME architectural area (wrong PCS consumption-site location; soft-delete fallback unviable post-MIGRATE; commit-gate unenforceable; backfill SQL shape unspecified) + 2 Reviewer WARNINGs (model surface cascade + JWT-drift invisible). User chose option (a) defer to design-only sprint. ADR-023 must settle: (1) PCS consumption-site location for snapshot re-resolution (NOT `MapSegmentToExportLinesAsync` which is payroll-only); (2) MIGRATE vs alternative determinism strategy for `agreement_code` + `employment_category`; (3) soft-delete consumption semantic post-decision; (4) backfill ledger pattern for UPDATE-with-backfill; (5) multi-commit ordering mechanical safety (not procedural); (6) User-model surface cascade strategy (25+ call sites); (7) JWT-drift admin-visible UI affordance. 4 tasks (TASK-3200 sprint open + TASK-3201 ADR-023 draft + TASK-3202 dual-lens ADR review + TASK-3203 sprint close); test totals expected unchanged at 833. Once ADR-023 lands ACCEPTED, S33 = Phase 4d-3 Part 2 implementation refinement reopens against settled design. | in-progress | 2026-05-16 → _pending_ | _pending_ (design-only; 833 carry-forward) | _pending_ |
